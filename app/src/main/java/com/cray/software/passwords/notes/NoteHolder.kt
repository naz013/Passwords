...
class NoteHolder(v: View, private val mEventListener: SimpleListener?) : RecyclerView.ViewHolder(v) {

    var binding: NoteListItemBinding? = null

    init {
        binding = DataBindingUtil.bind<T>(v)
        v.setOnClickListener { v1 ->
            mEventListener?.onItemClicked(adapterPosition, v1)
        }
