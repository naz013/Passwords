...
class HomeFragment : BaseFragment(), SyncListener, SimpleListener {
    ...
    fun loaderAdapter() {
        adapter = PasswordsRecyclerAdapter(DataProvider.getData(context, true, true))
    ...
    override fun endExecution(result: Boolean) {
    ...
    private fun showPasswordPopup(position: Int, view: View, el: ListInterface) {
        ...
        Utils.showLCAM(context, view, { item ->
            when (item) {
                0 -> copyText(getString(R.string.item_login), (el as PasswordListInterface).login)
                1 -> copyText(getString(R.string.item_password), (el as PasswordListInterface).password)
                2 -> edit(position)
    ...
    private fun showNotePopup(position: Int, view: View, el: ListInterface) {
        val items = arrayOf(getString(R.string.copy_note), getString(R.string.edit), getString(R.string.delete), getString(R.string.delete_with_backup))
        Utils.showLCAM(context, view, { item ->
            when (item) {
                0 -> copyText(getString(R.string.note), el.title)
                1 -> edit(position)
                2 -> delete(position, false)
                3 -> delete(position, true)
            }
        }, *items)
    }
    ...
    private fun edit(position: Int) {
        ...
        if (item is PasswordListInterface) {
            ...
            val intentId = Intent(context, CreateNoteActivity::class.java)
            intentId.putExtra(Constants.INTENT_ID, id)
            startActivity(intentId)
        }
    }
    ...
    private fun delete(position: Int, deleteFile: Boolean) {
        ...
        if (item is PasswordListInterface) {
            ...
            DeleteNoteTask(context, null).execute(del, if (deleteFile) 1L else 0L)
        }
